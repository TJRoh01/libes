...
use libes::enc::{Aes256Gcm, ChaCha20Poly1305, XChaCha20Poly1305};
...
#[test]
fn x25519_chachapoly1305_hmacsha256() {
    // Alias Ecies with algorithms
    type X25519ChaCha20Poly1305HmacSha256 = Ecies<X25519, ChaCha20Poly1305, HmacSha256>;
    ...
    let enc = X25519ChaCha20Poly1305HmacSha256::new(pk2);
    let ciphertext = enc.encrypt(LOREM_IPSUM).expect("encryption failed");

    // Decrypt
    let plaintext =
        X25519ChaCha20Poly1305HmacSha256::decrypt(sk2, &ciphertext).expect("decryption failed");

    assert_eq!(LOREM_IPSUM.to_vec(), plaintext);
}

#[test]
fn x25519_chachapoly1305_aead() {
    // Alias Ecies with algorithms
    type X25519ChaCha20Poly1305Aead = Ecies<X25519, ChaCha20Poly1305, Aead>;
    ...
    // Instantiate Ecies & Encrypt
    let enc = X25519ChaCha20Poly1305Aead::new(pk2);
    let ciphertext = enc.encrypt(LOREM_IPSUM).expect("encryption failed");

    // Decrypt
    let plaintext =
        X25519ChaCha20Poly1305Aead::decrypt(sk2, &ciphertext).expect("decryption failed");

    assert_eq!(LOREM_IPSUM.to_vec(), plaintext);
}

#[test]
fn x25519_chachapoly1305_syn() {
    // Alias Ecies with algorithms
    type X25519ChaCha20Poly1305Syn = Ecies<X25519, ChaCha20Poly1305, Syn>;
    ...
    // Instantiate Ecies & Encrypt
    let enc = X25519ChaCha20Poly1305Syn::new(pk2);
    let ciphertext = enc.encrypt(LOREM_IPSUM).expect("encryption failed");

    // Decrypt
    let plaintext =
        X25519ChaCha20Poly1305Syn::decrypt(sk2, &ciphertext).expect("decryption failed");
...
#[test]
fn ed25519_chachapoly1305_hmacsha256() {
    // Alias Ecies with algorithms
    type Ed25519ChaCha20Poly1305HmacSha256 = Ecies<Ed25519, ChaCha20Poly1305, HmacSha256>;
    ...
    // Instantiate Ecies & Encrypt
    let enc = Ed25519ChaCha20Poly1305HmacSha256::new(pk2);
    let ciphertext = enc.encrypt(LOREM_IPSUM).expect("encryption failed");

    // Decrypt
    let plaintext =
        Ed25519ChaCha20Poly1305HmacSha256::decrypt(sk2, &ciphertext).expect("decryption failed");

    assert_eq!(LOREM_IPSUM.to_vec(), plaintext);
}

#[test]
fn ed25519_chachapoly1305_aead() {
    // Alias Ecies with algorithms
    type Ed25519ChaCha20Poly1305Aead = Ecies<Ed25519, ChaCha20Poly1305, Aead>;
    ...
    // Instantiate Ecies & Encrypt
    let enc = Ed25519ChaCha20Poly1305Aead::new(pk2);
    let ciphertext = enc.encrypt(LOREM_IPSUM).expect("encryption failed");

    // Decrypt
    let plaintext =
        Ed25519ChaCha20Poly1305Aead::decrypt(sk2, &ciphertext).expect("decryption failed");

    assert_eq!(LOREM_IPSUM.to_vec(), plaintext);
}

#[test]
fn ed25519_chachapoly1305_syn() {
    // Alias Ecies with algorithms
    type Ed25519ChaCha20Poly1305Syn = Ecies<Ed25519, ChaCha20Poly1305, Syn>;
    ...
    // Instantiate Ecies & Encrypt
    let enc = Ed25519ChaCha20Poly1305Syn::new(pk2);
    let ciphertext = enc.encrypt(LOREM_IPSUM).expect("encryption failed");

    // Decrypt
    let plaintext =
        Ed25519ChaCha20Poly1305Syn::decrypt(sk2, &ciphertext).expect("decryption failed");
